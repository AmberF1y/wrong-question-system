...
```text
F-008 题目图片上传与管理已完成并通过 Pull Request #8 合并到 main
```

状态：

> F-001～F-004 的三表结构已成为 Flyway V1。F-005 通过 V2 增加当前复习状态、不可变复习历史、约束、索引和已有错题回填。F-008 复用 V1 已有的可空 `question.image_path`，没有新增或修改表结构，也没有回写 V1/V2。
...
路径中的题目 ID 来自数据库，文件名由 UUID 生成，扩展名来自服务端识别的
实际内容格式。客户端原文件名不参与磁盘路径。每次解析相对路径时都必须确认
规范化后的真实路径仍位于配置根目录内，并拒绝符号链接与目录逃逸。
...
F-008 没有修改数据库结构。它使用 V1 已有的 `question.image_path` 保存单张
图片的相对存储键，并在业务层协调数据库事务与本地文件系统：

- 上传成功后更新 `image_path`；
- 数据库回滚时删除新文件；
- 替换或移除提交后删除旧文件；
- 删除错题提交后删除图片文件；
- 数据库不保存原文件名、绝对路径或图片二进制。
...
# 34. F-008 图片路径与文件生命周期

`question.image_path` 的当前语义是“相对于配置图片根目录的受控存储键”，不是
可以直接公开访问的 URL，也不是用户提交的任意文件路径。

示例：

```text
questions/83/d9ef584d-a6b9-4524-b4eb-54183351f57e.gif
```

数据库与文件系统不能共享一个原子事务，因此 F-008 采用事务回调降低不一致
风险：

1. 上传内容先写入题目专属目录中的临时文件；
2. 校验完成后在同一文件系统内移动到 UUID 最终路径；
3. 数据库保存新相对路径；
4. 数据库回滚时删除新文件；
5. 数据库提交后删除被替换或移除的旧文件；
6. 删除错题时先保存旧路径，数据库提交后清理文件和空题目目录。

允许的图片格式为 PNG、JPEG、WebP、GIF，最大 20 MiB。文件格式由内容签名
识别，不依赖扩展名或 MIME。读取时再次验证相对路径、符号链接、实际格式和
文件大小；数据库路径对应的文件不存在时返回图片资源 404，不把整道错题
错误地视为不存在。
