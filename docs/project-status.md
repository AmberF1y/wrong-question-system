...
| F-009 | Completed | KaTeX 数学公式输入、预览和安全展示已实现，113 个前端测试、类型检查、构建、146 个后端测试和浏览器人工验收均已通过 |
| F-010 | Completed | 普通到期复习优先、每日一道已掌握题随机抽查、30 天同题冷却、失败重新入队、并发保护、自动化回归与浏览器人工验收均已完成 |
| R-001 | Completed | v1.0.0 已发布；PR、回归、真实使用、正式备份、标签、GitHub Release、计划归档和分支清理均已完成 |

F-010 开发分支：

```text
codex/mastered-random-spot-check
```

F-010 从 `d522f809` 创建独立功能分支。已实现普通到期队列清空后每日最多
一道已掌握题抽查、30 天同题冷却、稳定候选、四级抽查评价、失败按 1/3/7 天
重新入队、`SPOT_CHECK` 不可变历史和数据库唯一约束并发兜底。计划位于
...
R-001 从 `main@63a234f` 创建发布分支，只负责把当前已完成的核心闭环收口为
`v1.0.0`。本轮统一第一版产品范围，补齐 Windows 本地启动、数据备份和恢复
说明，执行发布回归与真实使用冒烟验证，最后创建带注释标签和 GitHub Release。
本轮不实现 Dashboard、薄弱知识点、趋势统计、OCR、部署或其他业务功能。
计划提交为 `fbfbb8a`。当前实现范围还包括把后端 Maven、前端 npm 与锁文件的
...
合并提交为 `541b359`；合并后的 `main` 全量回归通过，发布分支已删除。标签前
发布记录提交为 `99f31ec`；带注释标签 `v1.0.0` 和中文 GitHub Release 已创建，
标签对象为 `de310fc`，标签目标为 `99f31ec`，计划已归档。
...
一次性 baseline 和独立 MySQL 测试库的决策；ADR-003 记录 F-008 使用本地
文件系统、相对存储键和事务回调管理图片生命周期的决策。
...
安全路径解析以及提交/回滚后的文件清理。MySQL 复用已有可空
`question.image_path`，没有数据库迁移。前端实现了本地预览、文件校验、创建
部分成功恢复、编辑保持/替换/移除、详情大图与局部重试，以及每日复习题图。
...
在 `737fef8f154cbc1341bfea9faef81415cc8e4602` 上实际通过：

- 后端：146 个测试，0 failure、0 error、0 skipped；
- 前端：16 个测试文件、81 个测试；
- TypeScript 类型检查通过；
- Vite 生产构建通过，转换 1725 个模块；
- 工作区 clean。
...
生产构建存在单个主 chunk 超过 500 kB 的 Vite 警告，但构建退出码为 0；这是
既有前端打包优化项，不阻断 F-008。
...
- 无图创建，以及 PNG、JPEG、WebP、GIF 的选择、预览、上传、显示与替换；
- SVG 客户端不可选择、超限图片客户端拒绝、伪装 PNG 被服务端内容签名拒绝；
- 详情页响应式显示、大图查看、刷新和后端重启后持久读取；
- 创建文字成功而图片失败时只产生一道错题，并在编辑页只重试图片；
- 图片 GET 返回正确 MIME、`inline`、`nosniff` 和 `no-store`；
- 每日复习在查看答案前显示题图，并保持 F-007 的答案按需请求；
- 图片文件缺失时文字仍正常，图片区域显示失败并可在恢复文件后重试；
- 单独移除图片不改变文字、知识点和复习状态，重复移除返回 409；
- 删除错题清理图片、复习状态和复习记录。

验收临时错题 ID 82、83、84 最终均返回 404。开发库中本轮临时错题、关联、
复习状态和复习记录均为 0，验收存储目录剩余文件为 0，工作区 clean。
...
## 22. R-001 v1.0.0 发布完成状态

R-001 最终状态为 `Completed`。发布分支已经删除：
...
- 第一版范围与实际代码能力对齐，Dashboard、薄弱知识点和趋势统计移入后续版本；
- 后端 Maven、前端 npm 与锁文件版本元数据统一为 `1.0.0`；
- 新增 Windows PowerShell 5.1 本地启动脚本；
- 新增 MySQL 与题目图片配套备份脚本；
- 新增本地长期使用、人工恢复和故障处理说明；
- 新增 `v1.0.0` 中文发布说明草案。
...
- 两个脚本均通过 Windows PowerShell 5.1 解析；
- 缺少数据库密码、图片目录位于仓库内部和运行中备份保护均按约定拒绝操作；
- 使用固定图片目录启动成功，前后端均返回 200，停止和重启正常；
- 正式开发库已保留真实学习错题 ID 85，重启后核心字段与复习状态保持不变；
- “每日复习”页面正常，尚未到期时显示无待复习题符合业务规则；
- 首次正式备份目录为
  `D:\WrongQuestionBackups\wrong-question-system-20260906-093225`；
- 数据库转储为 11574 字节，清单 2 条记录逐条复核通过；
- 正式图片目录当时为空，因此源文件数与备份文件数均为 0；
- 发布分支后端 146 个测试、前端 16 个文件共 81 个测试、类型检查和生产构建通过；
- 验证前后 Git 工作区均 clean。

发布候选主要证据 SHA-256：

- 首次启动日志：`dcc76d5ed28cb64df0afcf13fa3e0b0a3308fdf814265dc36360b4ef03bee163`；
- 运行中备份保护日志：`8e22f321cbbfe777f2bc3ea6846524b203f4c4fd1b8b8fbadc969f8c0fc67857`；
- 正式备份日志：`71b4ef054b84a26a26e5608462f4ed5ac7c838f3976e81487a881a8b26bccc78`；
- 备份清单：`aa91cddfafd22560f5f7fad29f6be29d09ab6e85ae53faf5893ab4e08493d5b4`；
- 重启前摘要：`88ad328ca66f9189f86a37cb25721414f37c8b5788d8ed208769d0c2fd30a99b`；
- 重启日志：`b7769e1d300677e0461ce1ad9052c01b8a2337ad7f57b7198db852509a3da073`；
- 重启后摘要：`d74409265478a98a5661bd573ef2c648df41466e8a4d4191dc6a5cc4b901cc1d`；
...
- Release 已发布并标记为 `Latest`；
- 计划已归档到 `docs/plans/completed/R-001-v1-release.md`；
- 最终归档提交位于 `v1.0.0` 标签之后，只记录发布完成事实，不改变标签内的
  应用代码、数据库迁移、测试或版本元数据。
...
F-009 在不修改后端 API、数据库结构和既有纯文本存储方式的前提下，实现：

- `$...$` 行内公式、`$$...$$` 独立公式和 `\$` 美元符号转义；
- 普通文本、中文、公式与原有换行混排；
- 创建/修改错题共用的折叠式语法说明与五字段即时预览；
- 错题列表、详情、每日复习题目和答案面板的统一 KaTeX 渲染；
- 公式错误、分隔符未配对时回显原始文本；
- 普通文本安全转义、KaTeX `trust: false`、仅公式结果局部使用 `v-html`；
- 长独立公式在内容容器内横向滚动；
- 常见考研数学语法和八个泰勒展开式自动化测试。

当前已经通过前端 17 个测试文件共 113 个测试、TypeScript 类型检查、Vite
生产构建和后端 146 个测试，八个泰勒展开式均包含在通过的自动化用例中。
用户已于 2026-09-09 确认完成真实浏览器人工验收，F-009 标记为 Completed，
计划移动至 completed 目录。
...
F-010 在不引入自适应算法或用户配置的前提下，实现：

- 普通到期复习优先，当前科目范围没有到期题时才提供抽查；
- 全部科目每天合计最多完成一道已掌握题抽查；
- 同一道题完成抽查后进入 30 天冷却期；
- 同一天、同一科目范围和候选集合下稳定返回同一道题；
- 抽查题继续按需加载答案，并复用题图、KaTeX 和安全文本展示；
- 熟练保持 `MASTERED`，基本掌握、模糊和不会分别在 7、3、1 天后重新入队；
- 使用独立 `SPOT_CHECK` 历史事件、乐观锁和数据库唯一约束保护并发写入。

当前已经通过后端 167 个测试、前端 17 个测试文件共 119 个测试、TypeScript
类型检查和 Vite 生产构建。Flyway V3 已在独立测试数据库完成迁移和结构断言。
