...
| F-007 | In Progress | 每日复习前端闭环与已掌握错题重新加入入口；计划已确认，尚未开始实现 |
...
F-007 从 `main@6e466ee` 创建功能分支。用户已在真实本地仓库确认
`HEAD` 与 `origin/main` 均为
`6e466eee0039b66a72a03246203fc37ea1687233`、工作区 clean，且创建分支后
HEAD 未变化。本阶段仅开始写入计划文档，尚未编写前端或后端实现代码。
...
F-007 已进入 In Progress。当前只完成仓库阅读、范围确认、交互决策、异常恢复
设计、验收数据方案和功能分支创建；尚未新增每日复习页面、前端 review API、
四级评价控件或重新加入入口，也未产生新的测试结果。
...
- 现有后端 API 足够支撑每日复习闭环，默认不修改 Java、Flyway、数据库或 API 契约；
- 新增 `/reviews` 页面和“每日复习”导航；
- 默认全部科目，并支持单科目筛选；
- 初始只显示题目，点击后才请求完整错题详情；
- 展示我的错误答案、正确答案、解析和错误原因；
- 四级评价显示固定规则说明，但实际结果完全以后端响应为准；
- 评价成功后先显示结果，由用户点击进入下一题；
- 科目切换立即生效，评价提交中禁用切换；
- 评价断线或超时不自动重发，通过安全 GET 同步状态；
- 明确 404 或复习 409 后显示原因并重新获取服务器状态；
- 答案加载失败时保留当前题并允许重试；
- 已掌握错题详情提供重新加入入口；
- 重新加入前确认，成功后停留详情、刷新状态并提供前往每日复习入口；
- 真实浏览器验收使用精确临时 ID 和受控日期调整；
- 清理优先使用现有 API，最后只读核对五张表。

当前尚未实现：

- 每日复习页面、路由和导航；
- 前端 review 类型和 API；
- 查看答案、评价、结果、下一题和空队列交互；
- 不确定评价结果同步；
- 重新加入复习前端入口；
- F-007 自动化测试和浏览器验收。

F-007 仍明确排除 Dashboard、趋势统计、薄弱知识点、复习历史页面、图片上传、
