...
F-001～F-008 与 R-001 均已完成。R-001 已完成版本号、产品边界、本地长期
使用、正式备份、真实使用、发布回归、Pull Request #9 合并、带注释标签
`v1.0.0` 和中文 GitHub Release。标签准确指向最终发布内容提交 `99f31ec`，
项目第一版已正式发布。
...
仓库提供 Windows 本地可重复执行的 k6 性能测试套件。性能测试必须使用名称
符合安全规则的专用数据库、仓库外专用图片目录和 127.0.0.1:18080，脚本会在
发出请求前交叉核对数据库、图片目录、环境标识和 Git 提交。默认入口只运行
1 VU 的低负载读取基线；有限并发和写入场景均需要独立显式开关。

不要对 wrong_question_system、wrong_question_system_test、正式图片目录或
正式备份目录运行这些脚本。完整准备、运行、结果和复跑说明见
performance/README.md。

## 数学公式输入

题目、我的错误答案、正确答案、解析和错误原因都继续保存为普通文本，同时
支持在文字中嵌入 LaTeX：

- 行内公式使用 `$...$`，例如 `当 $x\to0$ 时，$\frac{\sin x}{x}\to1$。`；
- 独立公式使用 `$$...$$`，例如 `$$\int_a^b f(x)\,dx$$`；
- 需要显示普通美元符号时输入 `\$`；
- 没有公式分隔符的内容按原有纯文本方式显示，换行继续保留。

创建和修改错题时可展开“公式输入说明与预览”即时核对效果。系统只保存原始
LaTeX 文本，不保存生成的 HTML；未配对或无法解析的公式会回显原文。普通文本
始终由 Vue 安全转义，只有 KaTeX 在 `trust: false` 下生成的结果用于公式展示。
